...
def main(args):
    ...
    print("Loading data...")
    test_data = []
    data_file = os.path.join(args.data_dir, f"train.jsonl" if args.infer_train_set else f"test.jsonl")
    print(f"Reading from file: {data_file}")
    ...
    print(f"Model exists: {os.path.exists(args.model_name_or_path)}")
    print(f"Tokenizer path: {args.tokenizer_name_or_path}")
    print(f"Tokenizer exists: {os.path.exists(args.tokenizer_name_or_path)}")
    ...
    print(f"Final number of examples to process: {len(test_data)}")

    if not test_data:
        print("WARNING: No test data to process!")
        return
    ...
    try:
        ...
        print(f"Got {len(results)} inference results")
    except Exception as e:
        print(f"Error during inference: {str(e)}")
        raise e

    print(f"Save directory: {args.save_dir}")
    print(f"Save directory exists: {os.path.exists(args.save_dir)}")
    ...
    print(f"Model path exists: {os.path.exists(args.model_name_or_path)}")
    print(f"Tokenizer path exists: {os.path.exists(args.tokenizer_name_or_path)}")
